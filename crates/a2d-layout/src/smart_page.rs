...
pub fn smart_page_layout(paper: PaperSize, style: SmartPageStyle) -> PageLayout {
    ...
    // Smart Pages are loose pages with no gutter exclusion (that's Milestone 5.3's
    // bound-notebook-only concern) -- the left margin equals the other three edges' margin.
    build_layout(BuildLayoutParams {
        id,
        physical_size: paper.physical_size(),
        left_margin_mm: SAFE_MARGIN_MM,

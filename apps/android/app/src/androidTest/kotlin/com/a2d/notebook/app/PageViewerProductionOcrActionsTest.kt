...
class PageViewerProductionOcrActionsTest {
    ...
    fun pageViewerHydratesDetectedOcrFromRustReadback() {
        ...
        try {
            val scan = registerRealScan(client = client, root = root)
            val recorded = recordTerminalOcr(
                client = client,
                scan = scan,
                status = OcrRunStatus.DETECTED,
                text = "durable detected page viewer sentinel",
    ...
    fun pageViewerHydratesUnavailableOcrAndRetriesThroughDurableQueue() {
        ...
        try {
            ...
            val recorded = recordTerminalOcr(
                ...
                text = "",
                unavailableReason = OcrUnavailableReason.PROVIDER_FAILED,
            ...
            val retryJob = requireNotNull(client.claimNextOcrJob()) {
                "Retry OCR must enqueue a Rust-owned durable job"
            }
    ...
    fun routeReopenPreservesDurableRecognizedStateFromRust() {
        ...
        try {
            ...
            recordTerminalOcr(
                ...
                scan = scan,
                status = OcrRunStatus.DETECTED,
                text = "route reopen durable OCR sentinel",
                unavailableReason = null,
                unavailableMessage = null,
            )
    ...
    @Suppress("unused")
    private fun completeRunningJobWithOcrRun(
        client: A2dClient,
        jobId: String,
        ocrRunId: String,
        retryable: Boolean,
    ) = client.completeOcrJob(
        CompleteOcrJobRequest(
            jobId = jobId,
            ocrRunId = ocrRunId,
            retryable = retryable,
        ),

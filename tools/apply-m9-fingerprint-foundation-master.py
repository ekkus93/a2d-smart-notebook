...
fingerprint = Path("crates/a2d-image/src/fingerprint.rs")
if fingerprint.exists():
    raise SystemExit(f"refusing to overwrite existing file: {fingerprint}")
fingerprint.write_text(
    r'''use a2d_domain::{A2dError, ErrorCategory, ErrorCode, ErrorSeverity};

use crate::{ImageRotation, OwnedGrayImage};
...
/// Versioned, compact representation of an upright, already-rectified and contrast-normalized page.
///
...
impl PerceptualFingerprintV1 {
    pub fn from_gray_image(image: &OwnedGrayImage) -> Result<Self, A2dError> {
        if image.rotation() != ImageRotation::Degrees0 {
            return Err(fingerprint_error(
                "IMAGE_FINGERPRINT_ROTATION_UNSUPPORTED",
                "perceptual fingerprint source must be upright after canonical rectification",
            )
            .with_detail("rotation_degrees", image.rotation().degrees().to_string()));
        }
...
mod tests {
    ...
    fn image_with_cells_and_rotation(
        cells: impl Fn(usize, usize) -> u8,
        ...
        OwnedGrayImage::from_tight(width as u32, height as u32, rotation, bytes).unwrap()
    }

    fn image_with_cells(cells: impl Fn(usize, usize) -> u8) -> OwnedGrayImage {
        image_with_cells_and_rotation(cells, ImageRotation::Degrees0)
    }
    ...
    #[test]
    fn non_upright_input_is_rejected_instead_of_silently_becoming_incomparable() {
        let rotated = image_with_cells_and_rotation(|_, _| 180, ImageRotation::Degrees90);
        let error = PerceptualFingerprintV1::from_gray_image(&rotated).unwrap_err();
        assert_eq!(error.code.as_str(), "IMAGE_FINGERPRINT_ROTATION_UNSUPPORTED");
    }
}
'''
)

lib = Path("crates/a2d-image/src/lib.rs")
replace_once(lib, "mod error;\nmod input;", "mod error;\nmod fingerprint;\nmod input;")
replace_once(
    lib,
    '''pub use encoded::{
...
milestone9 = Path("crates/a2d-core/src/milestone9.rs")
replace_once(
    milestone9,
    "//! Milestone 9.1 durable scan registration.",
    "//! Milestone 9 durable scan registration and fingerprint foundation.",
)
replace_once(
    milestone9,
...
- [x] Versioned perceptual fingerprint. Rust stores a deterministic `mean-grid-16x24-v1` luminance
      signature derived only from an upright, aligned, contrast-normalized OCR image. The public API
      rejects non-upright input and exposes raw per-cell differences without inventing duplicate or
      revision thresholds.
- [ ] Aligned change-region comparison.
''',
)
